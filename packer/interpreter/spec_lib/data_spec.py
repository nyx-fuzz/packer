...
class StructDataType:
    ...
    def msgpack(self):
        return ["Struct", self.name, [[name, dtype.d_id]
                                      for (name, dtype) in self.fields]]

    def build_from_val(self, val):
        res = b""
        for (name, dtype) in self.fields:
            res += dtype.build_from_val(val[name])
        return res
...
class IntDataType:
    def __init__(self, d_id, name, size, generators=[]):
        ...
        self.c_type_def = "typedef %s %s;" % (
            "uint%d_t" % (size * 8), self.c_type_name)

    def msgpack(self):
        # print(repr((self.name,self.c_type_name,self.generators)))
        return ["Int", self.name, self.size, [g.msgpack()
    ...
    def build_from_val(self, val):
        if isinstance(val, (bytes, str)):
            assert(len(val) == 1)
            val = ord(val)
        packers = {1: "B", 2: "H", 4: "L", 8: "Q"}
        return struct.pack("<" + packers[self.size], val)
...
class VecDataType:
    def __init__(self, d_id, name, dtype, size_range, generators):
        ...
        self.c_type_def = "typedef struct {size_t count; %s* vals; } %s;" % (
            self.dtype.c_type_name, self.c_type_name)

    def msgpack(self):
        return ["Vec", self.name, [self.size_range[0], self.size_range[1]],
                self.dtype.d_id, [g.msgpack() for g in self.generators]]

    def build_from_val(self, val):
        res = b"" + struct.pack("H", len(val))
        for v in val:
            res += self.dtype.build_from_val(v)
        return res
