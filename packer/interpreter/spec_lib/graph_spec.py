...
class NodeType:
    ...
    def msgpack(self):
        ...
        return [self.name, atomic_id, [inp.id for inp in self.inputs], [bor.id for bor in self.borrows], [out.id for out in self.outputs], self.interactive ]
...
class Spec:
    ...
    def node_type(self, name, inputs=[], borrows=[], outputs=[], data=None, interact=False, codegen_args=None, code=None):
        if data:
            data.emit_reader = True
        node = NodeType(len(self.nodes), name, inputs, borrows, outputs, data, interact)
