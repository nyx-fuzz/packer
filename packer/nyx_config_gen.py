...
def gen_nyx_config(config):
    ...
    if config.argument_values["path_to_default_config"]:
        ...
        if config.argument_values["vm_type"] == "Kernel":
            data["default_config"] = config.config_values['DEFAULT_FUZZER_CONFIG_FOLDER'] + \
                "/default_config_kernel.ron"
        else:
            data["default_config"] = config.config_values['DEFAULT_FUZZER_CONFIG_FOLDER'] + \
    ...
    if config.argument_values["vm_type"] == "Kernel":
        ...
        raise Exception(
            "Unkown VM Type <%s>" %
            (config.argument_values["vm_type"]))
    ...
    if config.argument_values["s"]:
        ...
        for file_name in glob.glob(
                os.path.abspath(
                    config.argument_values["s"]) +
                "/*.bin"):
            shutil.copyfile(
                file_name, "%s/seeds/seed_%d.bin" %
                (config.argument_values["share_dir"], seed_counter))
            seed_counter += 1
        for file_name in glob.glob(
                os.path.abspath(
                    config.argument_values["s"]) +
...
def gen_default_configs(config):
    if config.argument_values["vm_type"] == "Kernel" and not os.path.isfile(
            config.config_values['DEFAULT_FUZZER_CONFIG_FOLDER'] +
            "/default_config_kernel.ron"):
        ...
        f.write(
            get_default_kernel_config(
                config.config_values['QEMU-PT_PATH'],
                config.config_values['KERNEL'],
                config.config_values['INIT_RAMFS']))
        f.close()

    if config.argument_values["vm_type"] == "Snapshot" and not os.path.isfile(
        ...
        f = open(
            ...
            "w")
        f.write(
            get_default_vm_config(
                config.config_values['QEMU-PT_PATH'],
                config.config_values['DEFAULT_VM_HDA'],
                config.config_values['DEFAULT_VM_PRESNAPSHOT']))
        f.close()
