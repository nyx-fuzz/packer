...
def check_elf(file):
    ...
    if not (
            b"ELF" not in output and b"executable" not in output and b"Intel" not in output):
    ...
    print(
        FAIL +
        ERROR_PREFIX +
        "File is not an Intel x86 / x86-64 executable..." +
        ENDC)
    return None
...
def checks(config):
    if not os.path.isdir(
            config.config_values["AGENTS-FOLDER"] +
            "/linux_x86_64-userspace/"):
        ...
        if not os.path.isdir(
            ...
            print(
                FAIL +
                ERROR_PREFIX +
                "Wrong path to \"AGENTS-FOLDER\" configured..." +
                ENDC)
            return False
        else:
            config.config_values["AGENTS-FOLDER"] = os.path.dirname(
                os.path.realpath(__file__)) + "/" + config.config_values["AGENTS-FOLDER"]
    return True
...
def compile_spec(spec_folder_path, interpreter_folder_path):
    if not interpreter_path_valid(interpreter_folder_path):
        print(
            FAIL +
            "Error: nyx interpreter path is incorrenct (*check nyx.ini*)!" +
            ENDC)
        return False
    ...
    execute(["python3", "nyx_net_spec.py"],
            spec_folder_path, print_output=True)
...
def compile(config):
    ...
    if config.argument_values["nyx_net_stdin"]:
        NYX_NET_STDIN = os.path.abspath(
            config.argument_values["nyx_net_stdin"])
        # print(NYX_NET_STDIN)
    else:
        NYX_NET_STDIN = None
    ...
    if config.argument_values["coverage"] == "instrumentation":
        ...
        raise Exception(
            "Unkown mode: %s" %
            (config.argument_values["Coverage"]))
    ...
    if not LEGACY_MODE and SPEC_FOLDER:
        if not compile_spec(
                SPEC_FOLDER,
                config.config_values["NYX-INTERPRETER-FOLDER"]):
            return
    ...
    if len(os.listdir(config.argument_values["output_dir"])) != 0:
        if config.argument_values["purge"]:
            ...
            print(FAIL + "Error: %s is not empty!" %
                  (config.argument_values["output_dir"]) + ENDC)
            return
    ...
    if not check_memlimit(
            config.argument_values["m"],
            target_architecture == "m32"):
        return

    print(
        OKGREEN +
        INFO_PREFIX +
        "Executable architecture is Intel " +
        target_architecture +
        "-bit ..." +
        ENDC)
    ...
    if IGNORE_UDP_PORT:
        os.environ["STDOUT_STDERR_DEBUG"] += "-DIGNORE_PORT=" + \
            str(IGNORE_UDP_PORT) + " "
    if PRE_PROCESS and PRE_PROCESS_ARGS:
        os.environ["STDOUT_STDERR_DEBUG"] += "-DUSE_PRE_PROCESS "
    if SET_CLIENT_UDP_PORT:
        os.environ["STDOUT_STDERR_DEBUG"] += "-DCLIENT_UDP_PORT=" + \
            str(SET_CLIENT_UDP_PORT) + " "

    if target_architecture == "64":
        objcopy_type = "elf64-x86-64"
        mode = "64"
        ld_type = "elf_x86_64"
        agent_folder = config.config_values["AGENTS-FOLDER"] + \
            "/linux_x86_64-userspace/bin64/"
        print(OKGREEN + INFO_PREFIX + "Recompiling..." + ENDC)

        execute(["bash", "compile_64.sh"], config.config_values["AGENTS-FOLDER"] +
                "/linux_x86_64-userspace/", print_output=True)
        ...
        ld_type = "elf_i386"
        agent_folder = config.config_values["AGENTS-FOLDER"] + \
            "/linux_x86_64-userspace/bin32/"
        execute(["bash", "compile_32.sh"], config.config_values["AGENTS-FOLDER"] +
                "/linux_x86_64-userspace/", print_output=True)

    if SPEC_FOLDER:
        copyfile(
            "%s/%s" %
            (SPEC_FOLDER, "nyx_net_spec.msgp"), "%s/%s" %
            (config.argument_values["output_dir"], "spec.msgp"))

    if NYX_NET_STDIN:
        copyfile(
            NYX_NET_STDIN, "%s/%s" %
            (config.argument_values["output_dir"], "stdin.input"))
    ...
    if PRE_PROCESS:
        ...
        pre_process_dependencies, pre_process_asan_lib, ld_linux = copy_dependencies(
            config, PRE_PROCESS, config.argument_values["output_dir"] + "/pre_process", ld_type, agent_folder, folder="pre_process/")
        ...
        copyfile(
            PRE_PROCESS,
            "%s/%s" %
            (config.argument_values["output_dir"] +
             "/pre_process",
             os.path.basename(PRE_PROCESS)))
        download_script += "./hget pre_process/%s pre_process/pre_process\n" % (
            os.path.basename(PRE_PROCESS))

    dependencies, asan_lib, ld_linux = copy_dependencies(
        config, config.argument_values["binary_file"], config.argument_values["output_dir"], ld_type, agent_folder)

    asan_executable = False
    if not asan_lib:
        asan_executable = is_asan_executable(
            config.argument_values["binary_file"])
    ...
    if SETUP_FOLDER:
        ...
        shutil.copytree(SETUP_FOLDER, "%s/%s" %
                        (config.argument_values["output_dir"], "setup"))
    ...
    copyfile(
        ...
        "%s/%s" %
        (config.argument_values["output_dir"],
         os.path.basename(hcat_file)))
    copyfile(
        hget_file,
        "%s/%s" %
        (config.argument_values["output_dir"],
         os.path.basename(hget_file)))
    copyfile(
        habort_file,
        "%s/%s" %
        (config.argument_values["output_dir"],
         os.path.basename(habort_file)))
    ...
    if LEGACY_MODE:
        ...
        copyfile(
            ld_preload_fuzz_file_legacy,
            "%s/%s" %
            (config.argument_values["output_dir"],
             os.path.basename(ld_preload_fuzz_file)))
    ...
    if ld_linux:
        download_script += "./%s ./target_executable %s" % (
            ld_linux, config.argument_values["args"])  # fixme
    else:
        # fixme
        download_script += "./target_executable %s" % (
            config.argument_values["args"])
    ...
    f.write(download_script.replace("./hget hcat",
                                    "./hget hcat_no_pt").replace("./hget habort",
                                                                 "./hget habort_no_pt").replace("./hget ld_preload_fuzz.so",
                                                                                                "./hget ld_preload_fuzz_no_pt.so"))
    f.close()

    print(
        OKGREEN +
        INFO_PREFIX +
        "NYX share-dir is ready -> %s" %
        (config.argument_values["output_dir"]))
