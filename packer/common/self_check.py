...
def check_if_nativ_lib_compiled():
    if not (
        os.path.exists(
            os.path.dirname(
            sys.argv[0]) +
            "/fuzzer/native/") and os.path.exists(
                "os.path.dirname(sys.argv[0])+" /
                "fuzzer/native/bitmap.so")) and not (
                    os.path.exists("fuzzer/native/") and os.path.exists("fuzzer/native/bitmap.so")):
        print(
            WARNING +
            WARNING_PREFIX +
            "bitmap.so file does not exist. Compiling..." +
            ENDC)

        current_dir = os.getcwd()
        os.chdir(os.path.dirname(sys.argv[0]))
        p = subprocess.Popen(
            ("gcc fuzzer/native/bitmap.c --shared -fPIC -O3 -o fuzzer/native/bitmap.so").split(" "),
...
def check_if_installed(cmd):
    ...
    try:
        ...
        print(
            FAIL +
            ERROR_PREFIX +
            "Package 'msgpack' is missing (Hint: `pip install msgpack`)!" +
            ENDC)
        return False
...
def check_version():
    if sys.version_info < (3, 0, 0):
        print(
            FAIL +
            ERROR_PREFIX +
            "This script requires python 3.0 or higher!" +
            ENDC)
        return False
    return True
...
def check_packages():
    if not check_if_installed("lddtree"):
        print(
            FAIL +
            ERROR_PREFIX +
            "Tool 'lddtree' is missing (Hint: run `sudo apt install pax-utils`)!" +
            ENDC)
        return False
    return True
    ...
    try:
        ...
        if(verbose):
            print(
                WARNING +
                WARNING_PREFIX +
                "Multi range tracing is not supported! Please upgrade to kernel 4.20-rc4!" +
                ENDC)
        ret = 1
    finally:
        fd.close()
...
def vmx_pt_check_addrn(config):
    if "ip3" in config.argument_values and config.argument_values["ip3"]:
        ip_ranges = 4
    elif "ip2" in config.argument_values and config.argument_values["ip2"]:
        ip_ranges = 3
    elif "ip1" in config.argument_values and config.argument_values["ip1"]:
        ip_ranges = 2
    elif "ip0" in config.argument_values and config.argument_values["ip0"]:
        ip_ranges = 1
    else:
        ip_ranges = 0
    ...
    if config.argument_values["macOS"]:
        if config.config_values["APPLE-SMC-OSK"] == "":
            print(
                FAIL +
                ERROR_PREFIX +
                "APPLE SMC OSK is missing in nyx.ini!" +
                ENDC)
            return False
    return True
...
def check_apple_ignore_msrs(config):
    if config.argument_values["macOS"]:
        try:
            ...
            if 'Y' not in f.read(1):
                print(
                    FAIL +
                    ERROR_PREFIX +
                    "KVM is not properly configured! Please execute the following command:" +
                    ENDC +
...
def check_nyx_ini():
    if not os.path.exists(
            os.path.dirname(
                sys.argv[0]) +
            "/nyx.ini") and not os.path.exists("nyx.ini"):
        from common.config import FuzzerConfiguration
        FuzzerConfiguration(skip_args=True).create_initial_config()
        print(
            WARNING +
            WARNING_PREFIX +
            "nyx.ini file does not exist. Creating..." +
            ENDC)
        return False
    return True
...
def check_qemu_version(config):
    if not config.config_values["QEMU_KAFL_LOCATION"] or config.config_values["QEMU_KAFL_LOCATION"] == "":
        print(
            FAIL +
            ERROR_PREFIX +
            "QEMU_KAFL_LOCATION is not set in nyx.ini!" +
            ENDC)
        return False
    ...
    if not os.path.exists(config.config_values["QEMU_KAFL_LOCATION"]):
        print(
            FAIL +
            ERROR_PREFIX +
            "QEMU-PT executable does not exists..." +
            ENDC)
        return False
...
def check_cpu_num(config):
    ...
    if int(config.argument_values["p"]) > int(multiprocessing.cpu_count()):
        print(FAIL +
              ERROR_PREFIX +
              "Only %d fuzzing processes are supported..." %
